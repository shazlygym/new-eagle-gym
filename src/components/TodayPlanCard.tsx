      ...
      {!progress.complete && next && (
        <button
          ...
          className="flex w-full items-center gap-3 border-t border-ink-500/50 bg-brand-500/10
                     px-4 py-3.5 text-start active:bg-brand-500/20"
        >
          <div className="min-w-0 flex-1">
          ...
          <Play size={20} className="shrink-0 text-brand-500" fill="currentColor" />
        </button>
      )}

      {/* Dots for the week, so a glance shows what is left. */}
      {!progress.complete && program.days.length > 1 && (
        <div className="flex gap-1.5 border-t border-ink-500/50 px-4 py-3">
